...
import { getUserInitialsFromFullName } from '@/lib/utils';
...
export default function RecentAppointments({ appointments, employees = [] }: RecentAppointmentsProps) {
  ...
  const getEmployeeName = (appointment: Appointment) => {
    return (appointment as any)?.employee_name || 'Unknown Employee';
  };

  const getReportStatus = (reportId?: string) => {
    if (!reportId) return { status: 'No Report', variant: 'secondary' as const };
    
    // For now, return pending status since we don't have reports data
  ...
  if (!appointments || appointments.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-muted-foreground">
        {!animationLoading && calendarAnimationData ? (
            ...
            <div className="flex-shrink-0">
              <Avatar className="h-10 w-10">
                <AvatarFallback className="bg-primary/10 text-primary text-sm font-semibold">
                  {getUserInitialsFromFullName(getEmployeeName(appointment))}
                </AvatarFallback>
              </Avatar>
            </div>
