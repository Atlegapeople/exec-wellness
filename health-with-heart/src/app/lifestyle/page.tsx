...
function LifestylePageContent() {
  ...
  const [selectedLifestyleId, setSelectedLifestyleId] = useRouteState<
    string | null
  >('selectedLifestyleId', null, { scope: 'path' });
  ...
  const openOrganizationsModal = () => {
    ...
    toast.info('Organizations modal opened');
  };
  ...
  const openLocationsModal = () => {
    ...
    toast.info('Locations modal opened');
  };
  ...
  const openCostCentersModal = () => {
    ...
    toast.info('Cost Centers modal opened');
  };
  ...
  const handleCreate = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.success('Lifestyle record created successfully!');
      } else {
        const error = await response.json();
        console.error('Create failed:', error);
        toast.error('Failed to create lifestyle record');
      }
    } catch (error) {
      console.error('Error creating lifestyle:', error);
      toast.error('Error creating lifestyle record. Please try again.');
    } finally {
  ...
  const handleEdit = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error('Failed to update lifestyle record');
      }
    } catch (error) {
      console.error('Error updating lifestyle:', error);
      toast.error('Error updating lifestyle record. Please try again.');
    } finally {
      setFormLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.ok) {
        ...
        await fetchAllLifestyles();
        toast.success('Lifestyle record deleted successfully!');
      } else {
        const error = await response.json();
        console.error('Delete failed:', error);
        toast.error('Failed to delete lifestyle record');
      }
    } catch (error) {
      console.error('Error deleting lifestyle:', error);
      toast.error('Error deleting lifestyle record. Please try again.');
    } finally {
      setFormLoading(false);
    }
  };
  ...
  const openCreateModal = () => {
    setFormData({});
    setIsCreateModalOpen(true);
    toast.info('Create Lifestyle modal opened');
  };
  ...
  const openEditModal = (lifestyle: Lifestyle) => {
    ...
    toast.info('Edit Lifestyle modal opened');
  };

  const openDeleteModal = (lifestyle: Lifestyle) => {
    setEditingLifestyle(lifestyle);
    setIsDeleteModalOpen(true);
    toast.warning('Delete Lifestyle modal opened');
  };

  // Modal close functions with toast notifications
  const closeCreateModal = () => {
    setIsCreateModalOpen(false);
    toast.info('Create Lifestyle modal cancelled');
  };

  const closeEditModal = () => {
    setIsEditModalOpen(false);
    toast.info('Edit Lifestyle modal cancelled');
  };

  const closeDeleteModal = () => {
    setIsDeleteModalOpen(false);
    toast.info('Delete Lifestyle modal cancelled');
  };

  const closeOrganizationsModal = () => {
    setIsOrganizationsModalOpen(false);
    toast.info('Organizations modal closed');
  };

  const closeSitesModal = () => {
    setIsSitesModalOpen(false);
    toast.info('Sites modal closed');
  };

  const closeLocationsModal = () => {
    setIsLocationsModalOpen(false);
    toast.info('Locations modal closed');
  };

  const closeCostCentersModal = () => {
    setIsCostCentersModalOpen(false);
    toast.info('Cost Centers modal closed');
  };
      ...
      <DashboardLayout>
        <div className='px-8 sm:px-12 lg:px-16 xl:px-24 py-8'>
          <Card>
            <CardContent>
              <PageLoading
                text='Loading Lifestyle Data'
                subtitle='Fetching lifestyle assessment data from OHMS database...'
              />
            </CardContent>
          </Card>
        </div>
    ...
    <DashboardLayout>
      <div className='px-8 sm:px-12 lg:px-16 xl:px-24 py-6'>
        ...
        <Dialog
          ...
          onOpenChange={open => {
            if (!open) closeOrganizationsModal();
            else setIsOrganizationsModalOpen(true);
          }}
        >
        ...
        {/* Sites Modal */}
        <Dialog
          open={isSitesModalOpen}
          onOpenChange={open => {
            if (!open) closeSitesModal();
            else setIsSitesModalOpen(true);
          }}
        >
          <DialogContent className='max-w-6xl max-h-[90vh] overflow-y-auto'>
            <DialogHeader>
        ...
        <Dialog
          ...
          onOpenChange={open => {
            if (!open) closeLocationsModal();
            else setIsLocationsModalOpen(true);
          }}
        >
          <DialogContent className='max-w-6xl max-h-[90vh] overflow-y-auto'>
            <DialogHeader>
        ...
        <Dialog
          ...
          onOpenChange={open => {
            if (!open) closeCostCentersModal();
            else setIsCostCentersModalOpen(true);
          }}
        >
          <DialogContent className='max-w-6xl max-h-[90vh] overflow-y-auto'>
            <DialogHeader>
