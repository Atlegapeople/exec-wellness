...
export default function LifestylePage() {
  ...
  const filterLifestyles = useCallback((lifestyles: Lifestyle[], search: string) => {
    if (!search) return lifestyles;
    
    return lifestyles.filter(lifestyle => {
      const employeeName = `${lifestyle.employee_name || ''} ${lifestyle.employee_surname || ''}`.trim();
  ...
  const handleDelete = async () => {
    try {
      setFormLoading(true);
      const response = await fetch(`/api/lifestyle?id=${editingLifestyle?.id}`, {
        method: 'DELETE'
      });
  ...
  const paginateLifestyles = useCallback((lifestyles: Lifestyle[], page: number, limit: number) => {
  ...
  const transitionToPage = useCallback(async (newPage: number) => {
    ...
    const paginated = paginateLifestyles(filteredLifestyles, newPage, pagination.limit);
  ...
  useEffect(() => {
    ...
    setDisplayedLifestyles(paginated);
  }, [allLifestyles, searchTerm, filterLifestyles, paginateLifestyles, pagination.limit, searchParams]);
  ...
  }, [searchParams, pagination.page, filteredLifestyles.length, transitionToPage]);
              ...
              <CardContent>
                {displayedLifestyles.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-foreground mb-2">No lifestyle records found</h3>
                    <p className="text-muted-foreground">
                      {searchTerm ? 'Try adjusting your search criteria.' : 'No lifestyle records available.'}
                    </p>
                  </div>
                ) : (
                        ...
                        {displayedLifestyles.map((lifestyle) => (
                          <TableRow 
                            ...
                            className={`cursor-pointer transition-colors hover:bg-muted/50 ${
                              selectedLifestyle?.id === lifestyle.id ? 'bg-muted border-l-4 border-l-primary' : ''
                            }`}
                            ...
                            <TableCell>
                              <div>
                                ...
                                <div className="text-sm text-muted-foreground truncate">
                                  {lifestyle.employee_work_email || lifestyle.employee_id}
                                </div>
                              </div>
                            </TableCell>
                            <TableCell>
                              <Badge variant={lifestyle.smoke ? "destructive" : "secondary"}>
                                {lifestyle.smoke ? 'Smoker' : 'Non-smoker'}
                              </Badge>
                            </TableCell>
                            <TableCell>
                              <Badge variant={
                                lifestyle.auditc_result?.includes('No Risk') ? 'secondary' :
                                lifestyle.auditc_result?.includes('Low Risk') ? 'secondary' :
                                lifestyle.auditc_result?.includes('Risk') && !lifestyle.auditc_result?.includes('No Risk') && !lifestyle.auditc_result?.includes('Low Risk') ? 'destructive' : 'outline'
                              }>
                                {lifestyle.auditc_result || 'N/A'}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-sm">{lifestyle.excercise_frequency || 'N/A'}</TableCell>
                            <TableCell className="text-sm">{formatDate(lifestyle.date_created)}</TableCell>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      ...
                      <CardDescription className="flex items-center gap-2">
                        <span className="text-lg font-medium">{getEmployeeName(selectedLifestyle)}</span>
                ...
                <CardContent className="space-y-6 max-h-[600px] overflow-y-auto scrollbar-premium">
                  ...
                  <div className="space-y-3">
                    <h3 className="font-semibold text-sm uppercase tracking-wide text-muted-foreground flex items-center gap-2">
                      ...
                      {selectedLifestyle.smoking_header || 'Smoking Information'}
                    </h3>
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Smoker:</span>
                        <Badge variant={selectedLifestyle.smoke ? "destructive" : "secondary"}>
                          {selectedLifestyle.smoke ? 'Yes' : 'No'}
                        </Badge>
                      </div>
                      {selectedLifestyle.smoke_qty && (
                        <div className="flex gap-2">
                          <span className="text-muted-foreground min-w-[120px]">Quantity:</span>
                          <span className="font-medium">{selectedLifestyle.smoke_qty}</span>
                        </div>
                      )}
                      {selectedLifestyle.smoke_years && (
                        <div className="flex gap-2">
                          <span className="text-muted-foreground min-w-[120px]">Years:</span>
                          <span className="font-medium">{selectedLifestyle.smoke_years}</span>
                        </div>
                      )}
                      {selectedLifestyle.smoking_duration && (
                        <div className="flex gap-2">
                          <span className="text-muted-foreground min-w-[120px]">Duration:</span>
                          <span className="font-medium">{selectedLifestyle.smoking_duration}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <h3 className="font-semibold text-sm uppercase tracking-wide text-muted-foreground flex items-center gap-2">
                      ...
                      {selectedLifestyle.alcohol_header || 'Alcohol Information'}
                    </h3>
                    <div className="space-y-3 text-sm">
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Frequency:</span>
                        <span className="font-medium">{selectedLifestyle.alochol_frequency || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Quantity:</span>
                        <span className="font-medium">{selectedLifestyle.alocohol_qty || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Excess:</span>
                        <span className="font-medium">{selectedLifestyle.alcohol_excess || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Total Score:</span>
                        <Badge variant="outline">{selectedLifestyle.alcohol_score || 0}</Badge>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">AUDIT-C Result:</span>
                        <Badge variant={
                          selectedLifestyle.auditc_result?.includes('No Risk') ? 'secondary' :
                          selectedLifestyle.auditc_result?.includes('Low Risk') ? 'secondary' :
                          selectedLifestyle.auditc_result?.includes('Risk') && !selectedLifestyle.auditc_result?.includes('No Risk') && !selectedLifestyle.auditc_result?.includes('Low Risk') ? 'destructive' : 'outline'
                        }>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Exercise:</span>
                        <span className="font-medium">{selectedLifestyle.exercise || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Frequency:</span>
                        <span className="font-medium">{selectedLifestyle.excercise_frequency || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Duration:</span>
                        <span className="font-medium">{selectedLifestyle.excercise_minutes || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Sitting Hours:</span>
                        <span className="font-medium">{selectedLifestyle.sitting_hours || 'N/A'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Eat Out:</span>
                        <span className="font-medium">{selectedLifestyle.eatout_frequency || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Fruit & Veg:</span>
                        <span className="font-medium">{selectedLifestyle.fruitveg_frequency || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Sugar:</span>
                        <span className="font-medium">{selectedLifestyle.sugar_consumption || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Overall:</span>
                        <Badge variant="outline">{selectedLifestyle.diet_overall || 'N/A'}</Badge>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Sleep Hours:</span>
                        <span className="font-medium">{selectedLifestyle.sleep_hours || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Sleep Rating:</span>
                        <Badge variant="outline">{selectedLifestyle.sleep_rating || 'N/A'}</Badge>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Rest Quality:</span>
                        <span className="font-medium">{selectedLifestyle.sleep_rest || 'N/A'}</span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData({...formData, smoke: value === 'true'})}
                >
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({...formData, smoke_qty: e.target.value})}
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData({...formData, alochol_frequency: value})}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="2-4 times/month">2-4 times a month</SelectItem>
                    <SelectItem value="2-3 times/week">2-3 times a week</SelectItem>
                    <SelectItem value="4+ times/week">4 or more times a week</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData({...formData, auditc_result: value})}
                >
                  <SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData({...formData, excercise_frequency: value})}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select frequency" />
                  </SelectTrigger>
                  <SelectContent>
                    ...
                    <SelectItem value="Once or twice a week">1-2 times per week</SelectItem>
                    <SelectItem value="3 times a week">3 times per week</SelectItem>
                    <SelectItem value="4 times a week">4 times per week</SelectItem>
                    <SelectItem value="Daily or more often">Daily or more</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData({...formData, excercise_minutes: value})}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select duration" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="I don't exercise">Don't exercise</SelectItem>
                    <SelectItem value="30 minutes">30 minutes</SelectItem>
                    <SelectItem value="60 minutes">60 minutes</SelectItem>
                    <SelectItem value="More than150 mins">More than 150 mins</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData({...formData, diet_overall: value})}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Good but could be better">Good but could be better</SelectItem>
                    <SelectItem value="Good with a large variety of fruit and veg, and low sugar">Excellent</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData({...formData, sleep_rating: value})}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    <SelectItem value="Poor">Poor</SelectItem>
                    <SelectItem value="Ok, could be better">Ok, could be better</SelectItem>
        ...
        <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Update the lifestyle assessment for {editingLifestyle ? getEmployeeName(editingLifestyle) : ''}.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="alochol_frequency_edit">Alcohol Frequency</Label>
                <Select
                  value={formData.alochol_frequency || ''}
                  onValueChange={(value) => setFormData({...formData, alochol_frequency: value})}
                >
              ...
              <div className="space-y-2">
                <Label htmlFor="excercise_frequency_edit">Exercise Frequency</Label>
                <Select
                  value={formData.excercise_frequency || ''}
                  onValueChange={(value) => setFormData({...formData, excercise_frequency: value})}
                >
              ...
              <div className="space-y-2">
                <Label htmlFor="excercise_minutes_edit">Exercise Duration</Label>
                <Select
                  value={formData.excercise_minutes || ''}
                  onValueChange={(value) => setFormData({...formData, excercise_minutes: value})}
                >
