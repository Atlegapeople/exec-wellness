...
export async function GET(
  ...
  try {
    ...
    // Get all related data using employee_id
    const vitalsQuery = `SELECT * FROM vitals_clinical_metrics WHERE employee_id = $1 ORDER BY date_created DESC LIMIT 1`;
    const medicalHistoryQuery = `SELECT * FROM employee_medical_history WHERE employee_id = $1 ORDER BY date_created DESC LIMIT 1`;
    const clinicalExamQuery = `SELECT * FROM clinical_examinations WHERE employee_id = $1 ORDER BY date_created DESC LIMIT 1`;
    const labTestsQuery = `SELECT * FROM lab_tests WHERE employee_id = $1 ORDER BY date_created DESC LIMIT 1`;
    const lifestyleQuery = `SELECT * FROM lifestyle WHERE employee_id = $1 ORDER BY date_created DESC LIMIT 1`;
    const mentalHealthQuery = `SELECT * FROM mental_health WHERE employee_id = $1 ORDER BY date_created DESC LIMIT 1`;
    const specialInvestigationsQuery = `SELECT * FROM special_investigations WHERE employee_id = $1 ORDER BY date_created DESC LIMIT 1`;
    const mensHealthQuery = `SELECT * FROM mens_health WHERE employee_id = $1 ORDER BY date_created DESC LIMIT 1`;
    ...
    const report = mainReport.rows[0] as any;
    const vitalsData = vitals.rows[0] as any;
    const historyData = medicalHistory.rows[0] as any;
    const examData = clinicalExam.rows[0] as any;
    const labData = labTests.rows[0] as any;
    const lifestyleData = lifestyle.rows[0] as any;
    const mentalData = mentalHealth.rows[0] as any;
    const specialData = specialInvestigations.rows[0] as any;
    const genderSpecificData =
      report.gender === 'Male'
        ? (mensHealth.rows[0] as any)
        : (womensHealth.rows[0] as any);
    const notesData = notes.rows[0] as any;
    ...
    const comprehensiveReport = {
      report_heading: {
        ...
        doctor_name: report.doctor_name
          ? report.doctor_name.includes('Dr.')
            ? report.doctor_name
      ...
      personal_details: {
        ...
        blood_pressure: vitalsData
          ? `${vitalsData.systolic_bp || '0'}\\${vitalsData.diastolic_bp || '0'}`
          : '0\\0',
        blood_pressure_status: vitalsData?.blood_pressure_status || '',
        bmi: vitalsData?.bmi || null,
        bmi_status: vitalsData?.bmi_status || '',
        whtr_percent: vitalsData?.whtr
          ? `${Math.round(vitalsData.whtr * 100 * 100) / 100}%`
          : '',
      ...
      clinical_examinations: {
        ...
        head_neck_incl_thyroid:
          examData?.['head_&_neck,_incl_thyroid'] || 'Not Done',
      ...
      lab_tests: {
        full_blood_count_an_esr:
          labData?.['full_blood_count_an_esr'] || 'Not Done',
        ...
        thyroid_stimulating_hormone:
          labData?.thyroid_stimulating_hormone || 'Not Done',
      ...
      medical_history: {
        ...
        inflammatory_bowel_disease: historyData?.inflammatory_bowel_disease
          ? 'Yes'
          : 'No',
        hepatitis: historyData?.hepatitis ? 'Yes' : 'No',
        surgery: historyData?.surgery ? 'Yes' : 'No',
        anxiety_or_depression: historyData?.anxiety_or_depression
          ? 'Yes'
          : 'No',
        bipolar_mood_disorder: historyData?.bipolar_mood_disorder
          ? 'Yes'
          : 'No',
        hiv: historyData?.hiv ? 'Yes' : 'No',
        tb: historyData?.tb ? 'Yes' : 'No',
        disability: historyData?.['Does this person have a disability?']
          ? 'Yes'
          : 'No',
        cardiac_event_in_family:
          historyData?.heart_attack || historyData?.heart_attack_60
            ? 'Yes'
            : 'No',
        cancer_family: historyData?.cancer_family ? 'Yes' : 'No',
      },
      allergies: {
        environmental: historyData?.environmental
          ? 'Yes'
          : historyData?.environmental === false
            ? 'No'
            : 'Unknown',
        food: historyData?.food
          ? 'Yes'
          : historyData?.food === false
            ? 'No'
            : 'Unknown',
        medication: historyData?.medication
          ? 'Yes'
          : historyData?.medication === false
            ? 'No'
            : 'Unknown',
      },
      ...
      screening: {
        abdominal_ultrasound: specialData?.abdominal_ultrasound
          ? 'Required'
          : 'Not Required',
        colonoscopy: specialData?.colonscopy_required
          ? 'Required'
          : 'Not Required',
        gastroscopy: specialData?.gastroscopy ? 'Required' : 'Not Required',
        bone_density_scan: specialData?.osteroporosis_screen
          ? 'Required'
          : 'Not Required',
        annual_screening_prostate:
          labData?.psa === 'Abnormal' ? 'Required' : 'Not Required',
      },
      mental_health: {
        anxiety_level:
          mentalData?.['gad2-score'] && mentalData['gad2-score'] <= 2
            ? 'LOW'
            : mentalData?.['gad2-score'] && mentalData['gad2-score'] <= 4
              ? 'MEDIUM'
              : 'HIGH',
        energy_level:
          mentalData?.energy_levels && mentalData.energy_levels <= 3
            ? 'LOW'
            : mentalData?.energy_levels && mentalData.energy_levels <= 6
              ? 'MEDIUM'
              : 'HIGH',
        mood_level: mentalData?.mood_feeling?.includes('Not at all')
          ? 'POSITIVE'
          : mentalData?.mood_feeling?.includes('Several days') ||
              mentalData?.mood_feeling?.includes('More than half')
            ? 'NEGATIVE'
            : 'UNKNOWN',
        stress_level:
          mentalData?.stress_level && mentalData.stress_level <= 3
            ? 'LOW'
            : mentalData?.stress_level && mentalData.stress_level <= 6
              ? 'MEDIUM'
              : 'HIGH',
        sleep_rating: lifestyleData?.sleep_rating?.includes('Good')
          ? 'GOOD'
          : lifestyleData?.sleep_rating?.includes('Fair')
            ? 'FAIR'
            : lifestyleData?.sleep_rating?.includes('Poor')
              ? 'POOR'
              : 'UNKNOWN',
      },
      cardiovascular_stroke_risk: {
        age_and_gender_risk:
          (age && age >= 45) || report.gender === 'Male'
            ? 'At Risk'
            : 'Low Risk',
        blood_pressure:
          vitalsData?.blood_pressure_status === 'High' ? 'At Risk' : 'Low Risk',
        cholesterol:
          labData?.total_cholesterol &&
          parseFloat(labData.total_cholesterol) > 5.0
            ? 'At Risk'
            : 'Low Risk',
        diabetes:
          (labData?.fasting_glucose &&
            parseFloat(labData.fasting_glucose) >= 7.0) ||
          historyData?.diabetes
            ? 'At Risk'
            : 'Low Risk',
        obesity: vitalsData?.bmi_status === 'Obese' ? 'At Risk' : 'Low Risk',
        waist_to_hip_ratio:
          vitalsData?.whtr_status === 'High' ? 'At Risk' : 'Low Risk',
        overall_diet: lifestyleData?.diet_overall?.includes('good')
          ? 'Low Risk'
          : 'At Risk',
        exercise:
          lifestyleData?.exercise?.includes("don't exercise") ||
          lifestyleData?.exercise?.includes('seldom')
            ? 'At Risk'
            : 'Low Risk',
        alcohol_consumption:
          lifestyleData?.alcohol_score > 0 ? 'At Risk' : 'No Risk',
        smoking: lifestyleData?.smoke ? 'At Risk' : 'No Risk',
        stress_level:
          mentalData?.stress_level <= 3
            ? 'Low Risk'
            : mentalData?.stress_level <= 6
              ? 'Medium Risk'
              : 'At Risk',
        previous_cardiac_event:
          historyData?.high_blood_pressure || historyData?.high_cholesterol
            ? 'At Risk'
            : 'Low Risk',
        cardiac_history_in_family:
          historyData?.heart_attack || historyData?.heart_attack_60
            ? 'At Risk'
            : 'Low Risk',
        stroke_history_in_family: historyData?.cancer_family
          ? 'At Risk'
          : 'Low Risk',
      ...
      mens_health: {
        recommendation_text:
          report.gender === 'Male'
            ? genderSpecificData?.recommendation_text || ''
            : '',
      },
      womens_health: {
        recommendation_text:
    ...
    const pdfBuffer = await renderToBuffer(
      React.createElement(ExecutiveMedicalReportPDF, {
