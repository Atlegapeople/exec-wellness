...
export async function GET() {
  try {
    ...
    const records: LifestyleRecord[] = result.rows as LifestyleRecord[];
...
function processLifestyleData(records: LifestyleRecord[]): LifestyleInsights {
  ...
  // Smoking analysis
  const smokingCounts = records.reduce(
    (acc, record) => {
      const smokes =
        record.smoke === 'True' ||
        record.smoke === 'true' ||
        (record.smoke as any) === true;
  ...
  const smokingRate = Math.round(
    (smokingCounts.smokers / records.length) * 100
  );
  ...
  // Alcohol analysis
  const alcoholFrequencyCounts = records.reduce(
    (acc, record) => {
  ...
  const alcoholRiskCounts = records.reduce(
    (acc, record) => {
  ...
  const averageScore =
    records
      ...
      records.filter(
        r => r.alcohol_score !== null && r.alcohol_score !== undefined
      ).length || 0;

  // Exercise analysis
  const exerciseFrequencyCounts = records.reduce(
    (acc, record) => {
  ...
  const exerciseDurationCounts = records.reduce(
    (acc, record) => {
  ...
  // Diet analysis
  const dietOverallCounts = records.reduce(
    (acc, record) => {
  ...
  const eatoutCounts = records.reduce(
    (acc, record) => {
  ...
  const fruitVegCounts = records.reduce(
    (acc, record) => {
  ...
  // Sleep analysis
  const sleepHoursCounts = records.reduce(
    (acc, record) => {
  ...
  const sleepQualityCounts = records.reduce(
    (acc, record) => {
  ...
  // Drugs analysis
  const drugsAnalysis = records.reduce(
    (acc, record) => {
  ...
  const drugUsageRate = Math.round(
    (drugsAnalysis.currentUsers / records.length) * 100
  );
