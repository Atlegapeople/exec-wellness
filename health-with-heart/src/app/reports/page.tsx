...
function ReportsPageContent() {
  ...
  const [createFormData, setCreateFormData] = useState<CreateReportFormData>(
    {}
  );
      ...
      if (employeeFilter && data.reports && data.reports.length > 0) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            setFormError(null);
            console.log(
              'Initial form data loaded successfully for report:',
              employeeReport.id
            );
          } else {
            const errorData = await response.json().catch(() => ({}));
            console.error(
              'Failed to fetch initial form data:',
              response.status,
              errorData
            );
            setFormError(
              `Failed to load form data: ${response.status} ${errorData.error || 'Unknown error'}`
            );
          }
        } catch (error) {
  ...
  const handleReportClick = async (report: MedicalReport) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData(data);
        setFormError(null);
        console.log('Form data loaded successfully for report:', report.id);
      } else {
        const errorData = await response.json().catch(() => ({}));
        console.error('Failed to fetch form data:', response.status, errorData);
        setFormData(null);
        setFormError(
          `Failed to load form data: ${response.status} ${errorData.error || 'Unknown error'}`
        );
  ...
  const handleSignReport = async (reportId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        toast.error(
          `Failed to sign report: ${errorData.message || 'Unknown error'}`
        );
      }
    } catch (error) {
      console.error('Error signing report:', error);
      toast.error('Error signing report. Please try again.');
    }
  };
  ...
  const closeCreateModal = () => {
    ...
    toast.warning('Create Report cancelled');
  };
  ...
  const handleCreate = async () => {
    try {
      ...
      if (
        !createFormData.patient_id ||
        !createFormData.appointment_id ||
        !createFormData.doctor_id
      ) {
        toast.error(
          'Please fill in all required fields (Patient, Appointment, and Doctor)'
        );
        return;
      }
      ...
      if (response.ok) {
        ...
        toast.error(
          `Failed to create report: ${error.error || error.message || 'Unknown error'}`
        );
      }
    } catch (error) {
  ...
  useEffect(() => {
    const restore = async () => {
      if (!selectedReport && selectedReportId && !loading) {
    ...
    restore();
  }, [selectedReportId, selectedReport, allReports, loading]);
  if (loading) {
    return (
      // <ProtectedRoute>
      <DashboardLayout>
                      ...
                      <div className='space-y-6'>
                        ...
                        <Card className='border-teal-200'>
                          ...
                          <CardContent className='pt-0'>
                            {/* Risk factors table */}
                            <div className='bg-background rounded border divide-y mb-6'>
      ...
      >
        <DialogContent className='max-w-4xl max-h-[90vh] overflow-y-auto'>
          <DialogHeader>
            ...
            <DialogDescription>
              Add a new executive medical report for a patient. All fields
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            ...
            <div className='space-y-2'>
              ...
              <Select
                ...
                onValueChange={value =>
                  setCreateFormData({ ...createFormData, patient_id: value })
                }
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  {/* You'll need to fetch patients list */}
            ...
            <div className='space-y-2'>
              ...
              <Select
                value={createFormData.appointment_id || ''}
                onValueChange={value =>
                  setCreateFormData({
                    ...createFormData,
              ...
              >
                ...
                <SelectContent>
                  {/* You'll need to fetch appointments list */}
                  <SelectItem value='appointment1'>Appointment 1</SelectItem>
                  <SelectItem value='appointment2'>Appointment 2</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className='space-y-2'>
              ...
              <Select
                ...
                onValueChange={value =>
                  setCreateFormData({ ...createFormData, doctor_id: value })
                }
              >
                <SelectTrigger>
            ...
            <div className='space-y-2'>
              ...
              <Input
                id='report_title'
                value={createFormData.report_title || ''}
                onChange={e =>
                  setCreateFormData({
                    ...createFormData,
            ...
            <div className='md:col-span-2 space-y-2'>
              ...
              <Textarea
                ...
                value={createFormData.executive_summary || ''}
                onChange={e =>
                  setCreateFormData({
                    ...createFormData,
            ...
            <div className='md:col-span-2 space-y-2'>
              ...
              <Textarea
                ...
                value={createFormData.clinical_findings || ''}
                onChange={e =>
                  setCreateFormData({
                    ...createFormData,
            ...
            <div className='md:col-span-2 space-y-2'>
              ...
              <Textarea
                id='assessment'
                value={createFormData.assessment || ''}
                onChange={e =>
                  setCreateFormData({
                    ...createFormData,
                ...
                placeholder='Enter medical assessment...'
                rows={3}
              />
            </div>
            ...
            <div className='md:col-span-2 space-y-2'>
              ...
              <Textarea
                id='recommendations'
                value={createFormData.recommendations || ''}
                onChange={e =>
                  setCreateFormData({
                    ...createFormData,
