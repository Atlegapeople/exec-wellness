...
export default function UsersPage() {
  ...
  const filterUsers = useCallback((users: UserWithMetadata[], search: string) => {
  ...
  const paginateUsers = useCallback((users: UserWithMetadata[], page: number, limit: number) => {
  ...
  useEffect(() => {
    ...
    setDisplayedUsers(paginated);
  }, [allUsers, searchTerm, filterUsers, paginateUsers, pagination.limit, searchParams]);
                          ...
                          >
                            <TableCell>
                              <div>
                                ...
                                <div className="text-sm text-muted-foreground">Created: {formatDate(user.date_created)}</div>
                              </div>
                            </TableCell>
                            <TableCell className="text-sm">{user.email}</TableCell>
                            <TableCell>
                              <Badge variant={getUserTypeBadgeVariant(user.type)} className="flex items-center gap-1 w-fit">
                                {getUserTypeIcon(user.type)}
                                {user.type}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-sm">{user.mobile || 'N/A'}</TableCell>
                            <TableCell>
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between pt-4 border-t gap-2">
                    ...
                    <div className="flex items-center space-x-1 flex-wrap">
                      ...
                      >
                        ...
                        <span className={`${selectedUser && leftWidth < 50 ? 'hidden' : 'hidden sm:inline'} ml-1`}>First</span>
                      </Button>
                      ...
                      >
                        ...
                        <span className={`${selectedUser && leftWidth < 50 ? 'hidden' : 'hidden sm:inline'} ml-1`}>Previous</span>
                      </Button>
                      
                      {/* Page Numbers */}
                      {Array.from({ length: Math.min(selectedUser && leftWidth < 50 ? 3 : 5, pagination.totalPages) }, (_, i) => {
                        const startPage = Math.max(1, pagination.page - (selectedUser && leftWidth < 50 ? 1 : 2));
                      ...
                      >
                        <span className={`${selectedUser && leftWidth < 50 ? 'hidden' : 'hidden sm:inline'} mr-1`}>Next</span>
                      ...
                      >
                        <span className={`${selectedUser && leftWidth < 50 ? 'hidden' : 'hidden sm:inline'} mr-1`}>Last</span>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      ...
                      <CardDescription className="flex items-center gap-2">
                        <Badge variant={getUserTypeBadgeVariant(selectedUser.type)} className="flex items-center gap-1">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      ...
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Mobile:</span>
                        <span className="font-medium">{selectedUser.mobile || 'N/A'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">User ID:</span>
                        <Badge variant="outline">{selectedUser.id}</Badge>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">User Type:</span>
                        <Badge variant={getUserTypeBadgeVariant(selectedUser.type)} className="flex items-center gap-1 w-fit">
                          {getUserTypeIcon(selectedUser.type)}
                          {selectedUser.type}
                        </Badge>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Signature:</span>
                        <span className="font-medium">{selectedUser.signature ? 'Available' : 'Not set'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      <div className="flex gap-2">
                        ...
                        <span className="font-medium">{formatDateTime(selectedUser.date_created)}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Created By:</span>
                        <span className="font-medium">{selectedUser.created_by_name || selectedUser.user_created || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Last Updated:</span>
                        <span className="font-medium">{formatDateTime(selectedUser.date_updated)}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Updated By:</span>
                        <span className="font-medium">{selectedUser.updated_by_name || selectedUser.user_updated || 'N/A'}</span>
                      </div>
                    </div>
                  </div>
                </CardContent>
