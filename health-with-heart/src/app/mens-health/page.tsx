...
function MensHealthPageContent() {
  ...
  const handleCreate = async () => {
    try {
      ...
      if (response.ok) {
        ...
        toast.success("Men's Health record created successfully!");
      } else {
        const error = await response.json();
        console.error('Create failed:', error);
        toast.error("Failed to create men's health record");
      }
    } catch (error) {
      console.error("Error creating men's health record:", error);
      toast.error("Error creating men's health record. Please try again.");
    } finally {
  ...
  const handleEdit = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setFormData({});
        await fetchAllMensHealth();
        toast.success("Men's Health record updated successfully!");
      } else {
        const error = await response.json();
        console.error('Update failed:', error);
        toast.error("Failed to update men's health record");
      }
    } catch (error) {
      console.error("Error updating men's health record:", error);
      toast.error("Error updating men's health record. Please try again.");
    } finally {
      setFormLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      if (response.ok) {
        ...
        await fetchAllMensHealth();
        toast.success("Men's Health record deleted successfully!");
      } else {
        const error = await response.json();
        console.error('Delete failed:', error);
        toast.error("Failed to delete men's health record");
      }
    } catch (error) {
      console.error("Error deleting men's health record:", error);
      toast.error("Error deleting men's health record. Please try again.");
    } finally {
      setFormLoading(false);
    }
  };
  ...
  const openCreateModal = () => {
    setFormData({});
    setIsCreateModalOpen(true);
    toast.info("Create Men's Health modal opened");
  };
  ...
  const openEditModal = (mensHealth: MenHealth) => {
    ...
    toast.info("Edit Men's Health modal opened");
  };

  const openDeleteModal = (mensHealth: MenHealth) => {
    setEditingMensHealth(mensHealth);
    setIsDeleteModalOpen(true);
    toast.warning("Delete Men's Health modal opened");
  };

  // Modal close functions with toast notifications
  const closeCreateModal = () => {
    setIsCreateModalOpen(false);
    toast.info("Create Men's Health modal cancelled");
  };

  const closeEditModal = () => {
    setIsEditModalOpen(false);
    toast.info("Edit Men's Health modal cancelled");
  };

  const closeDeleteModal = () => {
    setIsDeleteModalOpen(false);
    toast.info("Delete Men's Health modal cancelled");
  };
      ...
      <DashboardLayout>
        <div className='px-8 sm:px-12 lg:px-16 xl:px-24 py-8'>
          <Card>
            <CardContent>
              <PageLoading
                text="Loading Men's Health Data"
                subtitle="Fetching Men's Health data from OHMS database..."
              />
            </CardContent>
          </Card>
        </div>
