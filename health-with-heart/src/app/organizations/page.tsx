...
export default function OrganizationsPage() {
  ...
  const fetchOrganizations = async (page = 1, search = '') => {
    try {
      setLoading(true);
      const response = await fetch(`/api/organizations?page=${page}&limit=${pagination.limit}&search=${encodeURIComponent(search)}`);
  ...
  const handleViewManagers = (org: Organization) => {
    ...
    router.push(`/managers?organization=${orgFilter}&organizationName=${orgName}&returnUrl=${returnUrl}`);
  };
  ...
  const handleViewSites = (org: Organization) => {
    ...
    router.push(`/sites?organization=${orgFilter}&organizationName=${orgName}&returnUrl=${returnUrl}`);
  };
  ...
  const handleViewReports = (org: Organization) => {
    ...
    router.push(`/reports?organization=${orgFilter}&organizationName=${orgName}&returnUrl=${returnUrl}`);
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({...formData, notes_header: e.target.value})}
        ...
        <div className="orgs-container flex gap-0 overflow-hidden mb-6">
          ...
          >
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">With Registration</CardTitle>
                  <FileText className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">
                    {organizations.filter(org => org.registration_number).length}
                  </div>
                  <p className="text-xs text-muted-foreground">
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">Executive Medical</CardTitle>
                  <Users className="h-4 w-4 text-muted-foreground" />
                </CardHeader>
                <CardContent>
                  <div className="text-2xl font-bold">
                    {organizations.reduce((sum, org) => sum + parseInt(String(org.employee_count || 0)), 0)}
                  </div>
                  <p className="text-xs text-muted-foreground">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {organizations.reduce((sum, org) => sum + parseInt(String(org.manager_count || 0)), 0)}
                  </div>
                  <p className="text-xs text-muted-foreground">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {organizations.reduce((sum, org) => sum + parseInt(String(org.site_count || 0)), 0)}
                  </div>
                  <p className="text-xs text-muted-foreground">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {organizations.reduce((sum, org) => sum + parseInt(String(org.medical_report_count || 0)), 0)}
                  </div>
                  <p className="text-xs text-muted-foreground">
                        ...
                        >
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">
                              Executive Medical {(org.employee_count || 0) === 1 ? 'employee' : 'employees'}
                            </div>
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                            ...
                            <div className="text-xs text-muted-foreground">
                              {(org.manager_count || 0) === 1 ? 'manager' : 'managers'}
                            </div>
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-muted-foreground">
                              medical {(org.medical_report_count || 0) === 1 ? 'report' : 'reports'}
                            </div>
                          </div>
                          <div>
          ...
          <div 
            ...
            style={{ 
              width: selectedOrg ? `calc(${100 - leftPanelWidth}% - 4px)` : '0%',
              maxWidth: selectedOrg ? `calc(${100 - leftPanelWidth}% - 4px)` : '0%',
                  ...
                  <CardContent className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                      <div>
                        ...
                        <div className="font-semibold">{selectedOrg.name || 'Not specified'}</div>
                      </div>
                      <div>
                        <div className="text-muted-foreground">Registration Number</div>
                        <div className="font-semibold">{selectedOrg.registration_number || 'Not specified'}</div>
                      </div>
                      <div>
                        <div className="text-muted-foreground">Executive Medical Employees</div>
                        <div className="font-semibold flex items-center gap-2 mb-2">
                          <Users className="h-4 w-4 text-muted-foreground" />
                          {selectedOrg.employee_count || 0} {(selectedOrg.employee_count || 0) === 1 ? 'employee' : 'employees'}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="font-semibold flex items-center gap-2 mb-2">
                          <UserCheck className="h-4 w-4 text-muted-foreground" />
                          {selectedOrg.manager_count || 0} {(selectedOrg.manager_count || 0) === 1 ? 'manager' : 'managers'}
                        </div>
                      ...
                      <div>
                        <div className="text-muted-foreground">Sites/Workplaces</div>
                        <div className="font-semibold flex items-center gap-2 mb-2">
                          <MapPin className="h-4 w-4 text-muted-foreground" />
                          {selectedOrg.site_count || 0} {(selectedOrg.site_count || 0) === 1 ? 'site' : 'sites'}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="font-semibold flex items-center gap-2 mb-2">
                          <FileText className="h-4 w-4 text-muted-foreground" />
                          {selectedOrg.medical_report_count || 0} {(selectedOrg.medical_report_count || 0) === 1 ? 'report' : 'reports'}
                        </div>
                    ...
                    <CardContent className="space-y-4">
                      {selectedOrg.notes_header && (
                        <div>
                          ...
                          <div className="font-semibold">{selectedOrg.notes_header}</div>
                        </div>
                      )}
                      {selectedOrg.notes_text && (
                        <div>
                          <div className="text-muted-foreground">Notes</div>
                          <div className="font-medium whitespace-pre-wrap">{selectedOrg.notes_text}</div>
                        </div>
                      )}
                    </CardContent>
                  </Card>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <div className="font-semibold">{new Date(selectedOrg.date_created).toLocaleString()}</div>
                        <div className="text-sm text-muted-foreground">{selectedOrg.created_by_name || 'System'}</div>
                      </div>
                      <div>
                        <div className="text-muted-foreground">Last Updated</div>
                        <div className="font-semibold">{new Date(selectedOrg.date_updated).toLocaleString()}</div>
                        <div className="text-sm text-muted-foreground">{selectedOrg.updated_by_name || 'System'}</div>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="space-y-4">
                    {selectedOrg.logo ? (
                      <div className="space-y-4">
                        <div>
                          <div className="text-muted-foreground mb-2">Current Logo</div>
                          <div className="border rounded-lg p-4 bg-gray-50 flex justify-center">
                            <img 
                              ...
                              onError={(e) => {
                                console.error('Failed to load logo image:', selectedOrg.logo);
