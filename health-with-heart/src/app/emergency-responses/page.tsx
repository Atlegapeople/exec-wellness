...
export default function EmergencyResponsesPage() {
  ...
  const fetchEmergencyResponses = async (page = 1, search = '') => {
    try {
      ...
      if (response.ok) {
        ...
        setError(data.error || 'Failed to fetch emergency responses');
        setEmergencyResponses([]);
      }
    } catch (error) {
  ...
  const handleCreateResponse = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to create emergency response:', result.error);
        setFormErrors({
          general: result.error || 'Failed to create emergency response',
        });
      }
    } catch (error) {
  ...
  const handleEditResponse = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Failed to update emergency response:', result.error);
        setFormErrors({
          general: result.error || 'Failed to update emergency response',
        });
      }
    } catch (error) {
  ...
  const handleDeleteResponse = async (response: EmergencyResponse) => {
    ...
    try {
      ...
      if (deleteResponse.ok) {
        ...
        console.error('Failed to delete emergency response:', result.error);
      }
    } catch (error) {
  ...
  const openDeleteModal = (response: EmergencyResponse) => {
    setSelectedResponse(response);
                ...
                <div>
                  ...
                  {error.includes('does not exist') && (
                    <p className='text-xs mt-1'>
                      Please run the database schema first to create the
                      required tables.
                    </p>
                  )}
                </div>
                ...
                <div className='h-[650px] overflow-y-auto'>
                  <Table>
                    ...
                    <TableBody>
                      ...
                      {emergencyResponses.length === 0 && !error && (
                        <TableRow>
                          <TableCell
                            colSpan={7}
                      ...
                      <TabsContent value='basic' className='space-y-4 mt-4'>
                        ...
                        <Card>
                          ...
                          <CardContent className='space-y-2 text-sm'>
                            ...
                            <div>
                              <span className='font-medium'>Injury Time:</span>{' '}
        ...
        <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
          <DialogContent className='max-w-4xl max-h-[90vh] overflow-y-auto'>
            ...
            <div className='flex justify-end gap-2 pt-4'>
              ...
              <Button onClick={handleCreateResponse} disabled={isSubmitting}>
                {isSubmitting ? 'Creating...' : 'Create Emergency Response'}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
        ...
        <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
          <DialogContent className='max-w-4xl max-h-[90vh] overflow-y-auto'>
            ...
            <div className='flex justify-end gap-2 pt-4'>
              ...
              <Button onClick={handleEditResponse} disabled={isSubmitting}>
                {isSubmitting ? 'Updating...' : 'Update Emergency Response'}
              </Button>
            </div>
          </DialogContent>
        </Dialog>
            ...
            <div className='py-4'>
              ...
              {selectedResponse && (
                <div className='bg-gray-50 p-3 rounded-md text-sm'>
                  <p>
                    <strong>Employee:</strong> {selectedResponse.employee_name}{' '}
                    {selectedResponse.employee_surname}
                  </p>
                  <p>
                    <strong>Type:</strong> {selectedResponse.emergency_type}
                  </p>
                  <p>
                    <strong>Date:</strong>{' '}
                    {formatDate(selectedResponse.injury_date)}
                  ...
                  <p>
                    <strong>Location:</strong> {selectedResponse.place || 'N/A'}
                  </p>
                </div>
              )}
