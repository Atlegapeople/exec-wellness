...
import { AlertTriangle, Plus, Edit, Trash2, Search, Calendar, Clock, MapPin, User, FileText, Heart, Activity } from 'lucide-react';
...
export default function EmergencyResponsesPage() {
  ...
  const fetchEmergencyResponses = async (page = 1, search = '') => {
    try {
      ...
      console.log('Total emergency responses fetched:', data.emergencyResponses?.length || 0);
    ...
    try {
      ...
      if (!formData.employee_id || !formData.emergency_type) {
        setFormErrors({
          ...
          emergency_type: !formData.emergency_type ? 'Emergency type is required' : ''
        });
        return;
      }
  ...
  const handleDeleteResponse = async (response: EmergencyResponse) => {
    if (!confirm('Are you sure you want to delete this emergency response record?')) return;

    try {
      const deleteResponse = await fetch(`/api/emergency-responses?id=${response.id}`, {
        method: 'DELETE',
      });
  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Emergency Responses</h1>
                ...
                <Table>
                  ...
                  <TableBody>
                    {emergencyResponses.map((response) => (
                      ...
                      >
                        <TableCell>{formatDate(response.injury_date)}</TableCell>
                        <TableCell>
                          <div className="font-medium">
                            {response.employee_name} {response.employee_surname}
                          </div>
                        ...
                        <TableCell>
                          <Badge className={getEmergencyTypeBadgeColor(response.emergency_type)}>
                  ...
                  <Tabs defaultValue="basic" className="w-full">
                    ...
                    <TabsContent value="basic" className="space-y-4 mt-4">
                      <Card>
                        ...
                        <CardContent className="space-y-2 text-sm">
                          <div>
                            <span className="font-medium">Name:</span> {selectedResponse.employee_name} {selectedResponse.employee_surname}
                          </div>
                          <div>
                            <span className="font-medium">Email:</span> {selectedResponse.employee_work_email || 'N/A'}
                          </div>
                          <div>
                            <span className="font-medium">Employee ID:</span> {selectedResponse.employee_id}
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-2 text-sm">
                          <div>
                            ...
                            <Badge className={getEmergencyTypeBadgeColor(selectedResponse.emergency_type)}>
                              {selectedResponse.emergency_type || 'Unknown'}
                            </Badge>
                          </div>
                          <div>
                            <span className="font-medium">Injury Date:</span> {formatDate(selectedResponse.injury_date)}
                          </div>
                          <div>
                            <span className="font-medium">Injury Time:</span> {formatTime(selectedResponse.injury_time)}
                          </div>
                          <div>
                            <span className="font-medium">Arrival Time:</span> {formatTime(selectedResponse.arrival_time)}
                          </div>
                          <div>
                            <span className="font-medium">Location:</span> {selectedResponse.place || 'N/A'}
                          </div>
                        </CardContent>
                      </Card>
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-3 text-sm">
                          ...
                          <div>
                            <span className="font-medium">Manager:</span> {selectedResponse.manager || 'N/A'}
                          </div>
                        </CardContent>
                      </Card>
                    </TabsContent>
                    ...
                    <TabsContent value="medical" className="space-y-4 mt-4">
                      <Card>
                        ...
                        <CardContent className="space-y-3 text-sm">
                          <div>
                            <span className="font-medium">Main Complaint:</span>
                            <p className="mt-1 text-muted-foreground text-xs">
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-3 text-sm">
                          ...
                          <div>
                            <span className="font-medium">Reference:</span> {selectedResponse.reference || 'N/A'}
                          </div>
                          <div>
                            <span className="font-medium">Report ID:</span> {selectedResponse.report_id || 'N/A'}
                          </div>
                          <div>
                            <span className="font-medium">Created:</span> {formatDate(selectedResponse.date_created)}
                          </div>
                        </CardContent>
                      </Card>
                    </TabsContent>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    className={formErrors.employee_id ? 'border-red-500' : ''}
                  />
                ...
                <div>
                  ...
                  >
                    <SelectTrigger className={formErrors.emergency_type ? 'border-red-500' : ''}>
                  ...
                  {formErrors.emergency_type && (
                    <p className="text-sm text-red-600 mt-1">{formErrors.emergency_type}</p>
                  )}
                </div>
                <div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, injury_time: e.target.value }))}
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData(prev => ({ ...prev, sendemail: e.target.value }))}
                  />
                </div>
              </div>
            </TabsContent>
          ...
          <Tabs defaultValue="basic" className="w-full">
            ...
            <TabsContent value="basic" className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="edit_emergency_type">Emergency Type *</Label>
                  <Select
                    value={formData.emergency_type || ''}
                    onValueChange={(value) => setFormData(prev => ({ ...prev, emergency_type: value }))}
                  >
