...
export default function CostCentersPage() {
  ...
  const handleCreateCostCenter = async () => {
    try {
      ...
      const response = await fetch('/api/cost-centers', {
        ...
        body: JSON.stringify({
          ...formData,
          organisation_id: formData.organisation_id === 'none' ? null : formData.organisation_id,
  ...
  const handleEditCostCenter = async () => {
    ...
    try {
      setSubmitting(true);
      const response = await fetch(`/api/cost-centers/${selectedCostCenter.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleViewEmployees = (costCenter: CostCenter) => {
    ...
    const orgName = encodeURIComponent(costCenter.organisation_name || 'Organization');
    router.push(`/employees?organization=${orgFilter}&organizationName=${orgName}&returnUrl=${returnUrl}`);
  };

  const handleViewReports = (costCenter: CostCenter) => {
    const returnUrl = encodeURIComponent('/cost-centers');
    const costCenterFilter = encodeURIComponent(costCenter.id);
    const costCenterName = encodeURIComponent(`${costCenter.department || 'Department'} - ${costCenter.cost_center || 'Cost Center'}`);
    router.push(`/reports?costCenter=${costCenterFilter}&costCenterName=${costCenterName}&returnUrl=${returnUrl}`);
  };
    ...
    <DashboardLayout>
      <div className="pl-8 pr-[5vw] sm:pl-12 sm:pr-[6vw] lg:pl-16 lg:pr-[8vw] xl:pl-24 xl:pr-[10vw] py-6 max-w-full overflow-hidden">
          ...
          <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
            ...
            <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4 py-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="manager_contact_number">Manager Contact</Label>
                  <Input
                    id="manager_contact_number"
                    value={formData.manager_contact_number || ''}
                    onChange={(e) => setFormData({...formData, manager_contact_number: e.target.value})}
                    placeholder="Enter manager contact number"
                  />
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="person_responsible_for_account">Account Responsible Person</Label>
                  <Input
                    id="person_responsible_for_account"
                    value={formData.person_responsible_for_account || ''}
                    onChange={(e) => setFormData({...formData, person_responsible_for_account: e.target.value})}
                    placeholder="Enter responsible person name"
                  />
                </div>
                
                <div className="space-y-2 col-span-2">
                  <Label htmlFor="person_responsible_for_account_email">Account Responsible Email</Label>
                  <Input
                    id="person_responsible_for_account_email"
                    type="email"
                    value={formData.person_responsible_for_account_email || ''}
                    onChange={(e) => setFormData({...formData, person_responsible_for_account_email: e.target.value})}
                ...
                <div className="flex items-center space-x-2 col-span-2">
                  <input
                    ...
                    onChange={(e) => setFormData({...formData, manager_responsible: e.target.checked})}
                    className="rounded border border-input"
                  />
                  <Label htmlFor="manager_responsible">Manager is responsible for this cost center</Label>
                </div>
        ...
        <div className="cost-centers-container flex gap-1 min-h-[600px]">
          ...
          <div 
            className="space-y-4"
            style={{ width: selectedCostCenter ? `${leftPanelWidth}%` : '100%' }}
          >
            {/* Stats Cards */}
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {new Set(costCenters.map(cc => cc.department).filter(Boolean)).size}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Unique departments
                  </p>
              ...
              <Card>
                <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                  <CardTitle className="text-sm font-medium">Managed Centers</CardTitle>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {costCenters.reduce((sum, cc) => sum + (cc.employee_count || 0), 0)}
                  </div>
                  <p className="text-xs text-muted-foreground">
                    Across all centers
                  </p>
              ...
              <CardContent>
                {costCenters.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <h3 className="text-lg font-medium text-foreground mb-2">No cost centers found</h3>
                    <p className="text-muted-foreground">
                      {searchTerm ? 'Try adjusting your search criteria.' : 'No cost centers available.'}
                    </p>
                  </div>
                ) : (
                        ...
                        {costCenters.map((costCenter) => (
                          <TableRow 
                            ...
                            className={`cursor-pointer transition-colors hover:bg-muted/50 ${
                              selectedCostCenter?.id === costCenter.id ? 'bg-muted border-l-4 border-l-primary' : ''
                            }`}
                          ...
                          >
                            ...
                            <TableCell>
                              {costCenter.manager_name ? (
                                <div>
                                  <div className="font-medium">{costCenter.manager_name}</div>
                            ...
                            <TableCell>
                              <div className="flex items-center gap-2">
                                <span className="font-medium">{costCenter.employee_count || 0}</span>
                                <span className="text-sm text-muted-foreground">employees</span>
                              </div>
                            </TableCell>
                            <TableCell>
                              <div className="flex items-center gap-2">
                                <span className="font-medium">{costCenter.medical_report_count || 0}</span>
            ...
            >
              <Card className="glass-effect">
                ...
                <CardContent className="space-y-6 max-h-[600px] overflow-y-auto scrollbar-premium">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      <div className="flex gap-2">
                        ...
                        <span className="font-medium">{selectedCostCenter.department || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Cost Center:</span>
                        <span className="font-medium font-mono">{selectedCostCenter.cost_center || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Organization:</span>
                        <span className="font-medium">{selectedCostCenter.organisation_name || 'N/A'}</span>
                      </div>
                      {selectedCostCenter.workplace_address && (
                        <div className="flex gap-2 items-start">
                          <span className="text-muted-foreground min-w-[120px]">Address:</span>
                          <span className="font-medium">{selectedCostCenter.workplace_address}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  {/* Manager Information */}
                  {(selectedCostCenter.manager_name || selectedCostCenter.manager_email) && (
                    <div className="space-y-3">
                      <h3 className="font-semibold text-sm uppercase tracking-wide text-muted-foreground flex items-center gap-2">
                      ...
                      <div className="space-y-3 text-sm">
                        {selectedCostCenter.manager_name && (
                          <div className="flex gap-2 items-center">
                            ...
                            <div className="flex-1">
                              <div className="font-medium">{selectedCostCenter.manager_name}</div>
                        ...
                        {selectedCostCenter.manager_email && (
                          <div className="flex gap-2 items-center">
                            ...
                            <span className="font-medium text-xs break-all">{selectedCostCenter.manager_email}</span>
                          </div>
                        )}
                        {selectedCostCenter.manager_contact_number && (
                          <div className="flex gap-2 items-center">
                            <Phone className="h-4 w-4 text-muted-foreground" />
                            <span className="font-medium">{selectedCostCenter.manager_contact_number}</span>
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {(selectedCostCenter.person_responsible_for_account || selectedCostCenter.person_responsible_for_account_email) && (
                    <div className="space-y-3">
                      <h3 className="font-semibold text-sm uppercase tracking-wide text-muted-foreground flex items-center gap-2">
                      ...
                      <div className="space-y-3 text-sm">
                        {selectedCostCenter.person_responsible_for_account && (
                          <div className="flex gap-2">
                            ...
                            <span className="font-medium">{selectedCostCenter.person_responsible_for_account}</span>
                          </div>
                        )}
                        {selectedCostCenter.person_responsible_for_account_email && (
                          <div className="flex gap-2">
                            <span className="text-muted-foreground min-w-[60px]">Email:</span>
                            <span className="font-medium text-xs break-all">{selectedCostCenter.person_responsible_for_account_email}</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <div className="font-semibold flex items-center gap-2 mb-2">
                          <Users className="h-4 w-4 text-muted-foreground" />
                          {selectedCostCenter.employee_count || 0} {(selectedCostCenter.employee_count || 0) === 1 ? 'employee' : 'employees'}
                        </div>
                        <div className="text-xs text-muted-foreground mb-2">assigned to this cost center</div>
                        {(selectedCostCenter.employee_count || 0) > 0 && (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleViewEmployees(selectedCostCenter)}
                            className="flex items-center gap-2"
                          >
                      ...
                      <div>
                        <div className="text-muted-foreground">Medical Reports</div>
                        <div className="font-semibold flex items-center gap-2 mb-2">
                          <FileText className="h-4 w-4 text-muted-foreground" />
                          {selectedCostCenter.medical_report_count || 0} {(selectedCostCenter.medical_report_count || 0) === 1 ? 'report' : 'reports'}
                        </div>
                        <div className="text-xs text-muted-foreground mb-2">for employees in this cost center</div>
                        {(selectedCostCenter.medical_report_count || 0) > 0 && (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleViewReports(selectedCostCenter)}
                            className="flex items-center gap-2"
                          >
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      <div className="flex gap-2">
                        ...
                        <span className="font-medium">{formatDate(selectedCostCenter.date_created)}</span>
                      </div>
                      {selectedCostCenter.created_by_name && (
                        <div className="flex gap-2">
                          <span className="text-muted-foreground min-w-[120px]">Created By:</span>
                          <span className="font-medium">{selectedCostCenter.created_by_name}</span>
                        </div>
                      )}
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Last Updated:</span>
                        <span className="font-medium">{formatDate(selectedCostCenter.date_updated)}</span>
                      </div>
                      {selectedCostCenter.updated_by_name && (
                        <div className="flex gap-2">
                          <span className="text-muted-foreground min-w-[120px]">Updated By:</span>
                          <span className="font-medium">{selectedCostCenter.updated_by_name}</span>
                        </div>
                      )}
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Cost Center ID:</span>
                        <span className="font-mono text-xs">{selectedCostCenter.id}</span>
                      </div>
                    </div>
                  </div>
                </CardContent>
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4 py-4">
              ...
              <div className="space-y-2 col-span-2">
                <Label htmlFor="edit_workplace_address">Workplace Address</Label>
                <Textarea
              ...
              <div className="space-y-2">
                <Label htmlFor="edit_manager_contact_number">Manager Contact</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="edit_person_responsible_for_account">Account Responsible Person</Label>
                <Input
              ...
              <div className="space-y-2 col-span-2">
                <Label htmlFor="edit_person_responsible_for_account_email">Account Responsible Email</Label>
                <Input
              ...
              <div className="flex items-center space-x-2 col-span-2">
                <input
                  ...
                  className="rounded border border-input"
                />
                <Label htmlFor="edit_manager_responsible">Manager is responsible for this cost center</Label>
              </div>
