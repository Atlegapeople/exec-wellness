...
export default function Analytics() {
  ...
  const buildParams = (baseParams: string = '') => {
    ...
    if (baseParams) {
      return paramString ? `${baseParams}&${paramString}` : baseParams;
    }
    return paramString ? `?${paramString}` : '';
  };

  // Fetch analytics data with filtering
  const { data: staffData, loading: staffLoading, error: staffError } = useAPI(`/api/analytics/staff-productivity${buildParams()}`, [selectedOrganization, startDate, endDate]);
  const { data: outcomesData, loading: outcomesLoading, error: outcomesError } = useAPI(`/api/analytics/medical-outcomes${buildParams()}`, [selectedOrganization, startDate, endDate]);
  const { data: volumeData, loading: volumeLoading, error: volumeError } = useAPI(`/api/analytics/monthly-volume${buildParams()}`, [selectedOrganization, startDate, endDate]);
  const { data: employeeData, loading: employeeLoading, error: employeeError } = useAPI(`/api/analytics/employee-insights${buildParams()}`, [selectedOrganization, startDate, endDate]);
  const { data: medicalHistoryData, loading: medicalHistoryLoading, error: medicalHistoryError } = useAPI(`/api/analytics/medical-history${buildParams()}`, [selectedOrganization, startDate, endDate]);
  const { data: lifestyleData, loading: lifestyleLoading, error: lifestyleError } = useAPI(`/api/analytics/lifestyle${buildParams()}`, [selectedOrganization, startDate, endDate]);
  const { data: clinicalExaminationsData, loading: clinicalExaminationsLoading, error: clinicalExaminationsError } = useAPI(`/api/analytics/clinical-examinations${buildParams()}`, [selectedOrganization, startDate, endDate]);
  const { data: mentalHealthData, loading: mentalHealthLoading, error: mentalHealthError } = useAPI(`/api/analytics/mental-health${buildParams()}`, [selectedOrganization, startDate, endDate]);
      ...
      <div className="px-8 sm:px-12 lg:px-16 xl:px-24 py-8 space-y-8 animate-slide-up">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-4">
            ...
            {/* Filters Section */}
            <div className="flex flex-wrap items-end gap-4 p-4 border rounded-lg bg-muted/10">
