...
export default function ClinicalExaminationsAnalytics() {
  ...
  // Fetch data
  const { data: organizations } = useAPI<{ organizations: any[] }>('/api/organizations?limit=1000');
  const { data: clinicalData, loading, error, refetch } = useAPI(apiUrl, [selectedOrganization, startDate, endDate]) as { 
    data: ClinicalExamsData | null, 
    loading: boolean, 
    error: any, 
    refetch: () => void 
  };
  ...
  const exportToCSV = () => {
    ...
    const csvData = [
      ['Examination Type', 'Total Count', 'Positive', 'Negative', 'Not Done', 'Positive Rate %'],
    ...
    link.setAttribute('download', `clinical-examinations-${new Date().toISOString().split('T')[0]}.csv`);
          ...
          <div className="px-8 py-8">
            <div className="flex items-center justify-center min-h-96">
              <div className="text-center space-y-4">
                ...
                <h3 className="text-lg font-semibold">Loading Clinical Examinations Analytics</h3>
                <p className="text-muted-foreground">Processing examination data...</p>
              </div>
            </div>
          </div>
        </DashboardLayout>
            ...
            <Card className="border-destructive">
              <CardContent className="p-6">
                ...
                <p className="text-muted-foreground mb-4">
                  Failed to load clinical examinations data. Please check your database connection.
                </p>
        ...
        <div className="px-8 py-8 space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h1 className="text-3xl font-bold">Clinical Examinations Analytics</h1>
            </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none z-0">
                      <svg className="h-4 w-4 text-muted-foreground opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6 gap-4">
              <Card>
                <CardContent className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-muted-foreground">Total Examinations</span>
                  </div>
                  <div className="text-2xl font-bold mt-2">{clinicalData.summaryStats.total_examinations.toLocaleString()}</div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <div className="text-2xl font-bold mt-2">{clinicalData.summaryStats.total_employees.toLocaleString()}</div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-muted-foreground">Exam Types</span>
                  </div>
                  <div className="text-2xl font-bold mt-2">{clinicalData.summaryStats.examination_types}</div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-muted-foreground">Avg Positive Rate</span>
                  </div>
                  <div className="text-2xl font-bold mt-2">{clinicalData.summaryStats.average_positive_rate}%</div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-4">
                  ...
                  <div className="text-2xl font-bold mt-2">{clinicalData.summaryStats.organizations_count}</div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardContent className="p-4">
                  <div className="flex items-center gap-2">
                    <CheckCircle2 className="h-5 w-5" style={{ color: '#759282' }} />
                    <span className="text-sm font-medium text-muted-foreground">Top Exam</span>
                  </div>
                  <div className="text-sm font-bold mt-2 truncate" title={clinicalData.summaryStats.most_common_exam}>
              ...
              <CardContent>
                <div className="space-y-4">
                  {clinicalData.chartData.slice(0, 10).map((exam, index) => (
                    <div key={exam.examination_type} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        <h4 className="font-medium">{exam.examination_type}</h4>
                        <Badge variant="outline">{exam.total_count} total</Badge>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-4 text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-muted-foreground">({exam.positive_percentage}%)</span>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        <div className="w-full bg-gray-200 rounded-full h-2 overflow-hidden">
                          <div className="h-full flex">
                            <div 
                              ...
                              style={{ width: `${(exam.positive_count / exam.total_count) * 100}%` }}
                            ></div>
                            <div 
                              className="bg-red-500" 
                              style={{ width: `${(exam.negative_count / exam.total_count) * 100}%` }}
                            ></div>
                            <div 
                              className="bg-gray-400" 
                              style={{ width: `${(exam.not_done_count / exam.total_count) * 100}%` }}
                            ></div>
                          </div>
                        </div>
                      </div>
          ...
          {clinicalData?.organizationData && clinicalData.organizationData.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {clinicalData.organizationData.map((org, index) => (
                    <div key={org.organization_id} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <Badge 
                          variant={org.positive_rate >= 80 ? "default" : org.positive_rate >= 60 ? "secondary" : "destructive"}
                        >
                      ...
                      <div className="grid grid-cols-3 gap-4 text-sm">
                        <div>
                          ...
                          <div className="font-semibold">{org.total_examinations.toLocaleString()}</div>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Employees:</span>
                          <div className="font-semibold">{org.total_employees.toLocaleString()}</div>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Exam Types:</span>
                          <div className="font-semibold">{org.examination_types}</div>
                        </div>
                      </div>
                    </div>
                  ))}
                  ...
                  {clinicalData.trendData.map((trend, index) => (
                    <div key={trend.month} className="border rounded-lg p-4">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Badge variant="outline">{trend.total_examinations} exams</Badge>
                      </div>
                      
                      <div className="grid grid-cols-3 gap-4 text-sm">
                        <div>
                          <span className="text-muted-foreground">Employees:</span>
                          <div className="font-semibold">{trend.unique_employees}</div>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Positive:</span>
                          <div className="font-semibold text-green-600">{trend.positive_results}</div>
                        </div>
                        <div>
                          <span className="text-muted-foreground">Negative:</span>
                          <div className="font-semibold text-red-600">{trend.negative_results}</div>
                        </div>
                      </div>
                    </div>
                  ))}
