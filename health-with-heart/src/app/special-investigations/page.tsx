...
export default function SpecialInvestigationsPage() {
  ...
  const [allInvestigations, setAllInvestigations] = useState<SpecialInvestigation[]>([]);
  const [filteredInvestigations, setFilteredInvestigations] = useState<SpecialInvestigation[]>([]);
  const [displayedInvestigations, setDisplayedInvestigations] = useState<SpecialInvestigation[]>([]);
  ...
  const [selectedInvestigation, setSelectedInvestigation] = useState<SpecialInvestigation | null>(null);
  ...
  const [editingInvestigation, setEditingInvestigation] = useState<SpecialInvestigation | null>(null);
  ...
  const fetchAllInvestigations = async () => {
    try {
      setLoading(true);
      const url = new URL('/api/special-investigations', window.location.origin);
      ...
      console.log('Total investigations fetched:', data.investigations?.length || 0);
  ...
  const filterInvestigations = useCallback((investigations: SpecialInvestigation[], search: string) => {
    if (!search) return investigations;
    
    return investigations.filter(investigation => {
      const employeeName = `${investigation.employee_name || ''} ${investigation.employee_surname || ''}`.trim();
      ...
      return (
        ...
        investigation.employee_work_email?.toLowerCase().includes(searchLower) ||
  ...
  const handleDelete = async () => {
    try {
      setFormLoading(true);
      const response = await fetch(`/api/special-investigations?id=${editingInvestigation?.id}`, {
        method: 'DELETE'
      });
  ...
  const paginateInvestigations = useCallback((investigations: SpecialInvestigation[], page: number, limit: number) => {
  ...
  const transitionToPage = useCallback(async (newPage: number) => {
    ...
    const paginated = paginateInvestigations(filteredInvestigations, newPage, pagination.limit);
  ...
  useEffect(() => {
    ...
    setDisplayedInvestigations(paginated);
  }, [allInvestigations, searchTerm, filterInvestigations, paginateInvestigations, pagination.limit, searchParams]);
  ...
  }, [searchParams, pagination.page, filteredInvestigations.length, transitionToPage]);
        ...
        <Card className="w-96">
          <CardContent className="flex items-center justify-center py-12">
            <div className="text-center space-y-4">
              ...
              <p className="text-muted-foreground">Loading special investigations...</p>
            </div>
          </CardContent>
        </Card>
      </div>
                ...
                {displayedInvestigations.length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-muted-foreground">
                      {searchTerm ? 'Try adjusting your search criteria.' : 'No special investigations available.'}
                    </p>
                  </div>
                ) : (
                        ...
                        {displayedInvestigations.map((investigation) => (
                          <TableRow 
                            ...
                            className={`cursor-pointer transition-colors hover:bg-muted/50 ${
                              selectedInvestigation?.id === investigation.id ? 'bg-muted border-l-4 border-l-primary' : ''
                            }`}
                            ...
                            <TableCell>
                              <div>
                                ...
                                <div className="text-sm text-muted-foreground truncate">
                                  {investigation.employee_work_email || investigation.employee_id}
                                </div>
                              </div>
                            </TableCell>
                            <TableCell>
                              <Badge variant={
                                investigation.urine_dipstix === 'Normal' ? 'secondary' :
                                investigation.urine_dipstix === 'Abnormal' ? 'destructive' :
                                investigation.urine_dipstix === 'Irregularities but Acceptable' ? 'outline' : 'outline'
                              }>
                                {investigation.urine_dipstix || 'N/A'}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-sm">{investigation.lung_function || 'N/A'}</TableCell>
                            <TableCell>
                              <Badge variant={
                                investigation.risk_category === 'Low' ? 'secondary' :
                                investigation.risk_category === 'Medium' ? 'outline' :
                                investigation.risk_category === 'High' ? 'destructive' : 'outline'
                              }>
                                {investigation.risk_category || 'N/A'}
                              </Badge>
                            </TableCell>
                            <TableCell className="text-sm">{formatDate(investigation.date_created)}</TableCell>
                  ...
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      ...
                      <CardDescription className="flex items-center gap-2">
                        <span className="text-lg font-medium">{getEmployeeName(selectedInvestigation)}</span>
                        <Badge variant="outline">{selectedInvestigation.id}</Badge>
                      </CardDescription>
                    </div>
                    <Button
                      variant="ghost"
                ...
                <CardContent className="space-y-6 max-h-[600px] overflow-y-auto scrollbar-premium">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Resting ECG:</span>
                        <span className="font-medium">{selectedInvestigation.resting_ecg || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Stress ECG:</span>
                        <span className="font-medium">{selectedInvestigation.stress_ecg || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">VO2 Max:</span>
                        <span className="font-medium">{selectedInvestigation.predicted_vo2_max || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Body Fat %:</span>
                        <span className="font-medium">{selectedInvestigation.body_fat_percentage || 'N/A'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-3 text-sm">
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Urine Dipstick:</span>
                        <Badge variant={
                          selectedInvestigation.urine_dipstix === 'Normal' ? 'secondary' :
                          selectedInvestigation.urine_dipstix === 'Abnormal' ? 'destructive' : 'outline'
                        }>
                          {selectedInvestigation.urine_dipstix || 'N/A'}
                        </Badge>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Lung Function:</span>
                        <span className="font-medium">{selectedInvestigation.lung_function || 'N/A'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Risk Category:</span>
                        <Badge variant={
                          selectedInvestigation.risk_category === 'Low' ? 'secondary' :
                          selectedInvestigation.risk_category === 'Medium' ? 'outline' :
                          selectedInvestigation.risk_category === 'High' ? 'destructive' : 'outline'
                        }>
                          {selectedInvestigation.risk_category || 'N/A'}
                        </Badge>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Risk Score:</span>
                        <Badge variant="outline">{selectedInvestigation.risk_score || 0}</Badge>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">Reynolds Score:</span>
                        <span className="font-medium">{selectedInvestigation.reynolds_cardiovascular_risk_score || 'N/A'}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[140px]">Colonoscopy Required:</span>
                        <Badge variant={selectedInvestigation.colonscopy_required ? "default" : "secondary"}>
                          {selectedInvestigation.colonscopy_required ? 'Yes' : 'No'}
                        </Badge>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[140px]">Gastroscopy:</span>
                        <Badge variant={selectedInvestigation.gastroscopy ? "default" : "secondary"}>
                          {selectedInvestigation.gastroscopy ? 'Yes' : 'No'}
                        </Badge>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[140px]">Abdominal Ultrasound:</span>
                        <Badge variant={selectedInvestigation.abdominal_ultrasound ? "default" : "secondary"}>
                          {selectedInvestigation.abdominal_ultrasound ? 'Yes' : 'No'}
                        </Badge>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[140px]">Osteoporosis Screen:</span>
                        <Badge variant={selectedInvestigation.osteroporosis_screen ? "default" : "secondary"}>
                          {selectedInvestigation.osteroporosis_screen ? 'Yes' : 'No'}
                        </Badge>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-3 text-sm">
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">NerveIQ:</span>
                        <span className="font-medium">{selectedInvestigation.nerveiq || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">KardioFit:</span>
                        <span className="font-medium">{selectedInvestigation.kardiofit || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">NerveIQ CNS:</span>
                        <span className="font-medium">{selectedInvestigation.nerveiq_cns || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">NerveIQ Cardio:</span>
                        <span className="font-medium">{selectedInvestigation.nerveiq_cardio || 'N/A'}</span>
                      </div>
                      <div className="flex gap-2">
                        <span className="text-muted-foreground min-w-[120px]">NerveIQ Group:</span>
                        <span className="font-medium">{selectedInvestigation.nerveiq_group || 'N/A'}</span>
                      </div>
                    </div>
                  </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData({...formData, urine_dipstix: value})}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="Irregularities but Acceptable">Irregularities but Acceptable</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({...formData, lung_function: e.target.value})}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({...formData, stress_ecg: e.target.value})}
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setFormData({...formData, predicted_vo2_max: e.target.value})}
        ...
        <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Update the investigation record for {editingInvestigation ? getEmployeeName(editingInvestigation) : ''}.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="predicted_vo2_max_edit">Predicted VO2 Max</Label>
                <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="body_fat_percentage_edit">Body Fat Percentage</Label>
                <Input
              ...
              <div className="md:col-span-2 space-y-2">
                <Label htmlFor="recommendation_text_edit">Recommendations</Label>
                <Textarea
            ...
            <DialogHeader>
              ...
              <DialogDescription>
                ...
                <span className="font-medium">
                  {editingInvestigation ? getEmployeeName(editingInvestigation) : ''}
                </span>?
