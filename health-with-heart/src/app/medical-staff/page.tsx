...
function MedicalStaffPageContent() {
  ...
  const fetchStaff = async (page = 1, search = '') => {
    try {
      ...
      const response = await fetch(
        `/api/medical-staff?page=${page}&limit=${
          pagination.limit
      ...
      // Check signature status for all users in parallel (non-blocking)
      const signaturePromises = data.users.map(async (staffMember: any) => {
        try {
          const sigResponse = await fetch(
            `/api/medical-staff/check-signature?userId=${encodeURIComponent(
              staffMember.id
            )}`
          );
          if (sigResponse.ok) {
            const sigData = await sigResponse.json();
            return { id: staffMember.id, hasSignature: sigData.hasSignature };
          }
        } catch (error) {
          console.error(
            `Error checking signature for ${staffMember.id}:`,
            error
          );
        }
        return { id: staffMember.id, hasSignature: false };
      });

      // Wait for all signature checks to complete and update status
      const signatureResults = await Promise.all(signaturePromises);
      const statuses: Record<string, boolean> = {};
      signatureResults.forEach(result => {
        statuses[result.id] = result.hasSignature;
      });
  ...
  const handleCreateStaff = async () => {
    try {
      ...
      if (!response.ok)
        throw new Error('Failed to create medical staff record');
  ...
  const handleEditStaff = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error('Failed to update medical staff record');
    ...
    <DashboardLayout>
      <div className='pl-8 pr-[5vw] sm:pl-12 sm:pr-[6vw] lg:pl-16 lg:pr-[8vw] xl:pl-24 xl:pr-[10vw] py-6 max-w-full overflow-hidden'>
        ...
        <div className='staff-container flex gap-0 overflow-hidden mb-6'>
            ...
            <Card className='hover-lift flex-1 overflow-hidden'>
              ...
              <CardContent className='p-0'>
                <div className='overflow-hidden'>
                  {/* Sticky Header */}
                  <div className='bg-card border-b border-gray-200 px-6 py-3'>
                    ...
                    {loading ? (
                      ...
                      <div className='grid grid-cols-6 gap-4 px-6 py-8'>
                        <div className='col-span-6 text-center'>
                          ...
                          <p className='text-muted-foreground'>
                            No medical staff records match your search criteria.
                          </p>
                        </div>
                      </div>
                    ) : (
          ...
          <div
            className={`${
              selectedStaff ? 'animate-slide-up' : ''
            } overflow-hidden`}
            style={{
              width: selectedStaff
                ? `calc(${100 - leftPanelWidth}% - 4px)`
                : '0%',
              maxWidth: selectedStaff
                ? `calc(${100 - leftPanelWidth}% - 4px)`
                : '0%',
                    ...
                    <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
                      <div>
                        ...
                        <div className='font-semibold'>
                          {selectedStaff.name || 'Not specified'}
                        </div>
                      </div>
                      <div>
                        <div className='text-muted-foreground'>Last Name</div>
                        <div className='font-semibold'>
                          {selectedStaff.surname || 'Not specified'}
                        </div>
                      </div>
                      <div>
                        <div className='text-muted-foreground'>Type</div>
                        <div className='font-semibold'>
                          {selectedStaff.type || 'Not specified'}
                        </div>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <div className='font-semibold'>
                          {selectedStaff.license_number || 'Not specified'}
                        </div>
                      </div>
                      <div className='md:col-span-2'>
                        ...
                        <div className='font-semibold'>
                          {selectedStaff.email || 'Not specified'}
                        </div>
                      </div>
                      <div>
                        <div className='text-muted-foreground'>
                        ...
                        <div className='font-semibold'>
                          {selectedStaff.mobile || 'Not specified'}
                        </div>
                      </div>
                      <div>
                        <div className='text-muted-foreground'>
                        ...
                        <div className='font-semibold'>
                          {selectedStaff.phone || 'Not specified'}
                        </div>
                      </div>
                      <div>
                        <div className='text-muted-foreground'>
                      ...
                      <div className='md:col-span-3'>
                        ...
                        <div className='font-semibold'>
                          {selectedStaff.address || 'Not specified'}
                        </div>
                      </div>
                      <div>
                  ...
                  <CardContent>
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                      <div>
                        ...
                        <div className='font-semibold'>
                          {new Date(
                            selectedStaff.date_created
                          ).toLocaleString()}
                        </div>
                        <div className='text-sm text-muted-foreground'>
                          {selectedStaff.created_by_name || 'System'}
                        </div>
                      </div>
                      <div>
                        <div className='text-muted-foreground'>
                        ...
                        <div className='font-semibold'>
                          {new Date(
                            selectedStaff.date_updated
                          ).toLocaleString()}
                        </div>
                        <div className='text-sm text-muted-foreground'>
                          {selectedStaff.updated_by_name || 'System'}
                        </div>
                      </div>
                    </div>
                  </CardContent>
                    ...
                    {currentSignaturePath ? (
                      ...
                      <div className='text-center py-8 border-2 border-dashed border-gray-300 rounded-lg'>
                        ...
                        <p className='text-xs text-muted-foreground mt-1'>
                          Looking for: {selectedStaff.name}{' '}
                          {selectedStaff.surname}
                        </p>
                      </div>
                    )}
