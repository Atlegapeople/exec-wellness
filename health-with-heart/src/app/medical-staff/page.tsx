  ...
  const fetchStaff = async (page = 1, search = '') => {
    try {
      ...
      const response = await fetch(
        `/api/medical-staff?page=${page}&limit=${pagination.limit}&search=${encodeURIComponent(search)}`
      );
      ...
      for (const staffMember of data.users) {
        try {
          const sigResponse = await fetch(
            `/api/medical-staff/check-signature?userId=${encodeURIComponent(staffMember.id)}`
          );
          if (sigResponse.ok) {
                        ...
                        <div className='space-y-3'>
                          ...
                          <div className='text-xs text-muted-foreground mt-2'>
                            ...
                            <span className='font-medium'>
                              {selectedStaff.updated_by_name ||
                                selectedStaff.user_updated ||
                                'Unknown'}
                            </span>
                            <span> on </span>
                            <span className='font-medium'>
