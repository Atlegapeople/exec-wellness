...
export default function StaffProductivityChart({
  data,
      ...
      <div className='flex items-center justify-center h-64'>
        <div className='text-center'>
          ...
          <p className='text-sm text-muted-foreground mt-2'>
            Data length: {data?.staff?.length || 0}
          </p>
        </div>
      </div>
    );
  ...
  const topPerformers = staffData
    .sort((a, b) => b.signing_rate_pct - a.signing_rate_pct)
    .slice(0, 10);

  const doctorData = staffData.filter(item => item.staff_role === 'Doctor');
  const nurseData = staffData.filter(item => item.staff_role === 'Nurse');

  const totalReports = staffData.reduce(
    (sum, item) => sum + parseInt(item.total_reports?.toString() || '0'),
    0
  );
  const totalSigned = staffData.reduce(
    (sum, item) => sum + parseInt(item.signed_reports?.toString() || '0'),
    0
  );
  const totalUnsigned = staffData.reduce(
    (sum, item) => sum + parseInt(item.unsigned_reports?.toString() || '0'),
    0
  );
  const overallSigningRate =
    totalReports > 0 ? Math.round((totalSigned / totalReports) * 100) : 0;
    ...
    <div className='space-y-6'>
            ...
            <div className='flex items-center space-x-2'>
              ...
              <div>
                <p className='text-2xl font-bold'>
                  {doctorData.length + nurseData.length}
                </p>
                <p className='text-sm text-muted-foreground'>Medical Staff</p>
                <p className='text-xs text-muted-foreground'>
                  {doctorData.length} Doctors, {nurseData.length} Nurses
                </p>
              </div>
            </div>
          </CardContent>
            ...
            <ResponsiveContainer width='100%' height={300}>
              <BarChart
                data={[
                  {
                    role: 'Doctors',
                    total_reports: doctorData.reduce(
                      (sum, item) =>
                        sum + parseInt(item.total_reports?.toString() || '0'),
                      0
                    ),
                  ...
                  {
                    role: 'Nurses',
                    total_reports: nurseData.reduce(
                      (sum, item) =>
                        sum + parseInt(item.total_reports?.toString() || '0'),
                      0
                    ),
      ...
      <Card>
        ...
        <CardContent>
          <ResponsiveContainer width='100%' height={400}>
            <BarChart
              data={doctorData.sort(
                (a, b) => b.signing_rate_pct - a.signing_rate_pct
              )}
              margin={{
                top: 20,
                bottom: 60,
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Reports assigned to nurses (signing not implemented)
          </CardDescription>
        </CardHeader>
        <CardContent>
          {nurseData.length > 0 ? (
            ...
            <div className='flex items-center justify-center h-64'>
              <div className='text-center'>
                <p className='text-muted-foreground'>
                  No nurse productivity data available
                </p>
                <p className='text-sm text-muted-foreground mt-2'>
                  No nurses have reports assigned
                </p>
              </div>
            </div>
          )}
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Signed vs Unsigned reports by doctor
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width='100%' height={300}>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Medical staff with highest signing rates
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className='space-y-3'>
              {topPerformers.slice(0, 5).map((staff, index) => (
                <div
                  key={`staff-${index}-${staff.staff_name}`}
                ...
                >
                  <div className='flex items-center space-x-3'>
                    ...
                    <div>
                      ...
                      <p className='text-sm text-muted-foreground'>
                        {staff.signed_reports}/{staff.total_reports} reports
                        signed
                      </p>
                    </div>
                  </div>
                  <Badge
                    variant={
                      staff.signing_rate_pct >= 90
                        ? 'default'
                        : staff.signing_rate_pct >= 70
                          ? 'secondary'
                          : 'destructive'
                    }
                  >
        ...
        >
            ...
            <ResponsiveContainer width='100%' height={300}>
              <LineChart
                data={weeklyTrendData.map(item => ({
                  ...item,
                  week_start: new Date(item.week_start).toLocaleDateString(
                    'en-US',
                    {
                      month: 'short',
                      day: 'numeric',
          ...
          <CardFooter className='p-0 mt-4'>
            <div
              ...
              style={{ color: '#374151', fontSize: '12px' }}
            >
              Weekly trend of unsigned reports{' '}
