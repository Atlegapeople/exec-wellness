...
export default function RecentAppointments({
  ...
  const getReportStatus = (reportId?: string) => {
    if (!reportId || !reports)
      return { status: 'No Report', variant: 'secondary' as const };
    ...
    <div className='space-y-4'>
      {appointments.slice(0, 6).map(appointment => {
        ...
        return (
          <div
            key={appointment.id}
            className='flex items-start space-x-4 p-4 border rounded-lg hover:bg-muted/50 transition-colors hover-lift'
          >
