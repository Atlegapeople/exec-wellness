...
export default function WorkplaceHealthTable({
  data,
}: WorkplaceHealthTableProps) {
        ...
        <table className='w-full text-sm'>
          ...
          <tbody>
            {data.map((workplace, index) => {
              const fitnessRate = calculateFitnessRate(
                    ...
                    <div className='flex flex-col items-center'>
                      <div
                        className={`font-semibold ${
                          parseFloat(String(fitnessRate)) >= 95
                            ? 'text-green-600'
                            : parseFloat(String(fitnessRate)) >= 85
                              ? 'text-yellow-600'
                              : 'text-red-600'
                        }`}
                      >
