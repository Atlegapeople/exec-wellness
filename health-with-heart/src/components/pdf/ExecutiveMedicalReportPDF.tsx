...
export const ExecutiveMedicalReportPDF: React.FC<ExecutiveMedicalReportPDFProps> = ({ reportData }) => {
  ...
  const normalizeRiskStatus = (value: string): string => {
    ...
    if (lower.includes('no risk') || lower.includes('normal') || lower.includes('negative')) return 'No Risk';
    if (lower.includes('low risk') || lower.includes('low')) return 'Low Risk';
    if (lower.includes('medium') || lower.includes('moderate')) return 'Medium';
    if (lower.includes('at risk') || lower.includes('high') || lower.includes('abnormal')) return 'At Risk';
    return 'Low Risk';
  };

  const normalizeExamStatus = (value: string): string => {
    if (!value || value === 'Not Done' || value === 'Unknown') return 'Not Done';
    const lower = value.toLowerCase();
    if (lower.includes('normal') || lower.includes('negative')) return 'Normal';
    if (lower.includes('out of range') && lower.includes('acceptable')) return 'Out of Range, Acceptable';
    if (lower.includes('abnormal') || lower.includes('positive')) return 'Abnormal';
    return value;
  };
  ...
  const getBadgeStyle = (value: string) => {
    ...
    // Success badge
    if (lowerValue.includes('normal') || lowerValue.includes('no risk') || 
        lowerValue.includes('low risk') || lowerValue.includes('negative') ||
        lowerValue.includes('excellent') || lowerValue.includes('good') ||
        lowerValue.includes('very good')) {
      return styles.badgeSuccess;
    }
    
    // Warning badge
    if (lowerValue.includes('medium') || lowerValue.includes('out of range') ||
        lowerValue.includes('fair') || lowerValue.includes('not done') ||
        lowerValue.includes('elevated risk')) {
      return styles.badgeWarning;
    }
    
    // Danger badge
    if (lowerValue.includes('at risk') || lowerValue.includes('abnormal') ||
        lowerValue.includes('positive') || lowerValue.includes('high risk') ||
  ...
  const renderPieChart = () => {
    ...
    return (
      <View style={styles.riskChart}>
        <Text style={{ fontSize: 11, fontWeight: 'bold', marginBottom: 10 }}>Risk Distribution</Text>
